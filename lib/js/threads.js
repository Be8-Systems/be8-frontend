...
class Threads extends LitElement {
    ...
    render () {
        return repeat(this.threads, thread => thread.id, ({ endless, expire, nickname, sender, text, threadID, ts, type }) => {
            ...
            const endlessIcon = endless || isSystem ? html`<i class="fa-solid fa-check color-danger-font"></i>` : '';
            const senderID = isSystem ? '' : html`<span class="float-right">#${sender}<span></span>`;
            
            return html`<div expire="${expire}" id="${threadID}" sender="${sender}" type="${type}" class="thread hover-background">${icon}<div><p>${nickname}&nbsp;&nbsp;${endlessIcon}${senderID}</p><p>${text.substring(0, 23)}… <span class="float-right unselectable">${dateTime}</span></p></div></div>`; 
        });
    }
}

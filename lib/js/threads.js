...
class Threads extends LitElement {
    ...
    render () {
        const HTML = this.threads.map(({ endless, expire, nickname, sender, text, threadID, ts, type }) => {
            const icon = `<i class="fa-solid fa-${icons[type]} thread-avatar"></i>`;
            const endlessIcon = endless ? '<i class="fa-solid fa-check color-danger-font"></i>' : '';

            return `<div expire="${expire}" id="${threadID}" sender="${sender}" type="${type}" class="thread hover-background">${icon}<div><p>${nickname} ${endlessIcon}<span class="float-right">#${sender}<span></p><p>${text.substring(0, 9)}… <span class="float-right">${new Date(ts).toISOString()}</span></p></div></div>`;        
        }).join('');

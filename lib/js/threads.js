...
import { isPhone, icons, mockupPartner, domCache } from './data.js';
...
class Threads extends LitElement {
    ...
    render () {
        const HTML = this.threads.map(({ expire, nickname, sender, text, threadID, ts, type }) => {
            ...
            return `<div expire="${expire}" id="${threadID}" sender="${sender}" type="${type}" class="thread">${icon}<div><p>${nickname} <span class="float-right">#${sender}<span></p><p>${text.substring(0, 9)}… <span class="float-right">${new Date(ts).toISOString()}</span></p></div></div>`;        
        }).join('');

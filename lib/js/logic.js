...
async function decryptMessages (cipherMessages, detail) {
    const proms = cipherMessages.map(async function (message) {
        ...
        if (message.messageType === 'text') {
            const text = await be8.decryptTextSimple(sender, be8.getAccID(), message.text, message.iv);
            message.text = text;
        }
        if (message.messageType === 'image') {
            const content = await be8.decryptImageSimple(sender, be8.getAccID(), message.text, message.iv);
...
async function getThreads () {
    ...
    const dialogIDs = threads.filter(t => !t.groupID && t.sender !== 's1').map(t => t.sender);
...
app.addEventListener('writeMessage', async function ({ detail }) {
    ...
    const { cipherText, iv } = await be8.encryptTextSimple(sender, detail.convPartner.id, detail.message);

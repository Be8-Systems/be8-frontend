...
async function decryptMessages (cipherMessages, detail) {
    const proms = cipherMessages.map(async function (message) {
        ...
        const text = await be8.decryptTextSimple(detail.sender, be8.getAccID(), message.text, message.iv);
...
async function getThreads () {
    ...
    const decryptMessages = await decryptMessages(threads, detail);

    app.setThreads(decryptMessages);

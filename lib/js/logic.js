...
async function decryptMessages (cipherMessages, detail) {
    ...
    const proms = cipherMessages.map(async function (message) {
        const sender = message.groupVersionKey || (detail?.sender || message.sender);
        ...
        if (message.messageType === 'text') {
            ...
            const text = await be8.decryptTextSimple(sender, receiver, message.text, message.iv);

            message.text = text;
        }
        if (message.messageType === 'image') {
            const content = await be8.decryptImageSimple(sender, receiver, message.text, message.iv);

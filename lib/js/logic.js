...
async function groupJoin (detail) {console.log(detail);
...
async function newMessage (detail) {
    if (detail.threadID === app.getConversationPartner().threadID) {
        const options = { 
            detail: {
                ...detail,
                ...(detail.type === 'group' ? { groupID: detail.threadID } : {})
            } 
        };
...
async function decryptImages (messages) {
    ...
    images.forEach((imageMessage) => {
        ...
        return be8.decryptImageSimple(idForPublicKey, idForPrivateKey, imageMessage.content, imageMessage.iv).then(content => app.insertImage({
            ...imageMessage,
            content
        }));
    });
...
async function syncPublicKeys (extra = []) {
    const cachedIDs = await getCachedUserIDs();  console.log(extra);  console.log(cachedIDs);

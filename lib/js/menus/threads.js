...
class Threads extends LitElement {
    ...
    render () {
        return repeat(this.threads, thread => thread.id, ({ endless, expire, nickname, sender, text, threadID, ts, type }) => {
            ...
            return html`<div expire="${expire}" id="${threadID}" sender="${sender}" type="${type}" class="thread hover-background">${icon}<div><p>${nickname}&nbsp;&nbsp;${endlessIcon}${senderID}</p><p>${sanText} <span class="float-right unselectable">${dateTime}</span></p></div></div>`; 
        });
    }
}

...
class Messages extends LitElement {
    ...
    renderWriteContainer () {
        return html`<div class="write-container"><textarea @keydown="${this.writeMessage}" class="write-message-input" type="text" maxlength="1000"><div><i @click="${this.uploadMedia}" class="fa-solid fa-photo-film hover-font"></i><input class="hide" @change="${this.changeInputEvent}" type="file" accept="image/png, image/gif, image/jpeg"></div></div>`;
    }
    ...
    #renderTextContent ({ text }, timeIndicator) {
        ...
        if (urls.length === 0) {
            return html`<p class="message-text">${text.replaceAll('\n', '<br/>')} ${timeIndicator}</p>`;
        }
    ...
    #renderMessageContent (message, timeIndicator) {
        if (message.messageType === 'image') {
            return html`<i class="fa-solid fa-spin fa-circle-notch"></i><img style="height:500px" data-contentid="${message.contentID}" data-rendered="false" src=""><p>${timeIndicator}</p>`;
        }
    ...
    #renderUserMessage (message, isGroup) {
        ...
        return html`<div alt="${nickname}" data-messageid="${messageID}" class="message-container"><div class="message ${amIsender ? 'sent-message' : 'received-message'} message-${message.messageType}">${firstLine}${secondLine}</div></div>`; 
    }

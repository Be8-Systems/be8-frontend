...
class Messages extends LitElement {
    ...
    #renderStatusIndicator (amIsender, status, isGroup) {
        ...
        if (isGroup && this.#userGroupModal.members.every(m => status.includes(m.id) || m.id === this.ME.id)) {
            return html`&nbsp;&nbsp;<i @click="${() => this.#viewStatus(status, isGroup)}" class="fa-solid fa-check read-check"></i><i @click="${() => this.#viewStatus(status, isGroup)}" class="fa-solid fa-check read-check"></i>`;
        }
        if (isGroup) {
            return html`&nbsp;&nbsp;<i @click="${() => this.#viewStatus(status, isGroup)}" class="fa-solid fa-check sent-check"></i>`;
        }

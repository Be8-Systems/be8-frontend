...
class User extends LitElement {
        ...
        const tokenEvent = new CustomEvent('setToken', {
            ...
            detail: {
                ...
                success: ({ tokenType, validTime }) => {
                    const time = tokenType === 'endless' ? tokenType : `${validTime / 1000 / 60 / 60 / 24} days`;
                    domCache.toast.notification = {
                        type: 'success',
    ...
    render () {
        ...
        const status = html`<div class="settings-container"><p>Status</p><textarea @keydown="${e => this.keyDownStatus(e)}" maxlength="280">${this.ME.userStatus}</textarea></div>`;
        const endlessToken = this.ME.endless ? '' : html`<div class="settings-container"><p>Token</p><input type="text" maxlength="${tokenLength}"><button class="hover-green" @click="${this.clickOnEndlessToken}">Check</button></div>`;

...
class ConversationModal extends Modal {
    ...
    #clickOnGoToGroup () {
        animateMainToSide(this.#modalContent, this.#createGroup, this.#groupNameInput);
    }

    #clickOnBackToMain () {
        animateSideToMain(this.#modalContent, this.#createGroup, this.#dialogInput);
    }
    ...
    render () {
        const content = html`<p>${LANG.CONVERSATION}</p><input @keydown="${(e) => this.#keyDownOn1to1(e)}" tabindex="0" type="number" min="0"><div @click="${this.#clickOnGoToGroup}" class="sub-modal-button hover-background">Create a group <i class="fa-solid fa-arrow-right float-right"></i></div>`;
        const groupsettings = html`<div><p>Name</p><input @keydown="${(e) => this.#keyDownOnGroup(e)}" type="text" maxlength="20"></div><p>Type</p><select><option value="public">public</option><option value="private">private</option></select><button @click="${this.#clickOnCreateGroup}">create group</button>`;

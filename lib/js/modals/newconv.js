...
class ConversationModal extends Modal {
    ...
    clickOnGoToGroup () {
        this.#modalContent.style = 'animation: fadeLeftAnimation 0.3s !important;';

        setTimeout(() => {
            this.#modalContent.classList.add('hide');
            this.#createGroup.classList.remove('hide');
            this.#createGroup.style = 'animation: fadeRightAnimation 0.3s !important;';
    ...
    render () {
        const content = html`<p>${LANG.CONVERSATION}</p><input @keydown="${(e) => this.keyDownOn1to1(e)}" tabindex="0" type="text"><div @click="${this.clickOnGoToGroup}" class="create-group hover-background">Create a group <i class="fa-solid fa-arrow-right float-right"></i></div>`;

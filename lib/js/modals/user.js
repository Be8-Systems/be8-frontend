...
class Usermodal extends Modal {
    ...
    #renderNonSystem () {
        ...
        return html`${hl}${id}${nickname}${status}${expire}${endless}`;
    }
    ...
    #renderSystemCard () {
        ...
        return html`${hl}<p>This is a system user you can't send messages to the system user.</p>`;
    }

    render () {
        const isSystem = this.conversationPartner.type === 'system';
        const content = isSystem ? this.#renderSystemCard() : this.#renderNonSystem();
